...
public class ChannelRouteBuilder extends ErrorHandlerRouteBuilder {
    ...
    private void transferRoutes() {
        from("direct:get-transfer-transaction-id").id("transfer-details")
                .log(LoggingLevel.INFO, "## CHANNEL -> inbound transferDetail request for ${header.transactionId}").process(e -> {
                    ...
                    exchange = restTemplate.exchange(operationsUrl + "/transfers?page=0&size=20&transactionId=" + transactionId,
                    ...
                    if (contents.length() != 1) {
                        ...
                        exchange = restTemplate.exchange(operationsUrl + "/transfer/" + workflowInstanceKey, HttpMethod.GET, entity,
                                String.class);
                        ...
                        if (channelRequest != null) {
                            JSONObject channelRequestJson = new JSONObject(
                                    channelRequest.substring(1, channelRequest.length() - 1).replace("\\\"", "\""));
        ...
        from("direct:get-txnState-correlationId").id("transfer-details-correlationId")
                .log(LoggingLevel.INFO, "## CHANNEL -> request for txn with client correlation Id" + " ${header.X-CorrelationID}")
                .process(e -> {
                    String tenantId = e.getIn().getHeader("Platform-TenantId", String.class);
                    if (tenantId == null || !dfspIds.contains(tenantId)) {
                        throw new RuntimeException("Requested tenant " + tenantId + " not configured in the connector!");
    ...
    private void transactionRoutes() {
        ...
        from("direct:post-transaction-resolve").id("transaction-resolve").log(LoggingLevel.INFO, "## operator transaction resolve")
                .process(e -> {
                    ...
                    zeebeClient.newPublishMessageCommand().messageName(OPERATOR_MANUAL_RECOVERY)
                            .correlationKey(e.getIn().getHeader(TRANSACTION_ID, String.class)).timeToLive(Duration.ofMillis(30000))
                            .variables(variables).send();
                }).setBody(constant((Object) null));
    }

    private void partyRegistrationRoutes() {
        from("direct:post-party-registration").id("inbound-party-registration-request")
                .log(LoggingLevel.INFO, "## CHANNEL -> PHEE inbound party registration request").unmarshal()
                .json(JsonLibrary.Jackson, RegisterAliasRequestDTO.class).to("bean-validator:request").process(exchange -> {
                    String tenantId = exchange.getIn().getHeader("Platform-TenantId", String.class);
                    if (tenantId == null || !dfspIds.contains(tenantId)) {
                        throw new RuntimeException("Requested tenant " + tenantId + " not configured in the connector!");
                    }
                    ...
                    zeebeProcessStarter.startZeebeWorkflow(tenantSpecificBpmn, objectMapper.writeValueAsString(channelRequest), variables);
                }).setBody(constant((Object) null));
    }

    private void jobRoutes() {
        from("direct:post-job-resolve").id("job-resolve").log(LoggingLevel.INFO, "## operator job resolve").process(e -> {
            JSONObject request = new JSONObject(e.getIn().getBody(String.class));
            ...
            zeebeClient.newSetVariablesCommand(incident.getLong("elementInstanceKey")).variables(newVariables).send();

            zeebeClient.newUpdateRetriesCommand(incident.getLong("jobKey")).retries(incident.getInt("newRetries")).send();

            zeebeClient.newResolveIncidentCommand(incident.getLong("key")).send();
        }).setBody(constant((Object) null));

    }

    private void workflowRoutes() {
        from("direct:post-workflow-resolve").id("workflow-resolve").log(LoggingLevel.INFO, "## operator workflow resolve").process(e -> {
            JSONObject request = new JSONObject(e.getIn().getBody(String.class));
            JSONObject incident = request.getJSONObject("incident");
            Map<String, Object> newVariables = new HashMap<>();
            JSONObject requestedVariables = request.getJSONObject("variables");
        ...
        from("direct:post-workflow-instanceKey-cancel").id("workflow-cancel")
                .log(LoggingLevel.INFO, "## operator workflow cancel ${header.workflowInstanceKey}").process(e -> zeebeClient
                        .newCancelInstanceCommand(Long.parseLong(e.getIn().getHeader("workflowInstanceKey", String.class))).send())
                .setBody(constant((Object) null));
    }

    private void acknowledgementRoutes() {
        String id = "invoke-ack-workers";
        from("direct:" + id).id(id).log(LoggingLevel.INFO, "## CHANNEL -> Ack transaction request").process(exchange -> {
            ...
            for (String transactionId : transactionIds) {
                ...
                String instanceId = zeebeProcessStarter.startZeebeWorkflow(tenantSpecificBpmn, exchange.getIn().getBody(String.class),
                        extraVariables);
                JSONObject response = new JSONObject();
    ...
    private void inboundTransferC2bImplementationRoutes() {
        ...
        from("direct:post-gsma-transaction").id("gsma-transfer").unmarshal().json(JsonLibrary.Jackson, GsmaTransfer.class)
                .log(LoggingLevel.INFO, "GSMA Transfer Body").process(e -> {
    ...
    public UriComponentsBuilder buildParams(Client client) {
        ...
        UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(restAuthHost + "/oauth/token")
                // Add query parameter
                .queryParam("username", client.getClientId()).queryParam("password", client.getClientSecret())
